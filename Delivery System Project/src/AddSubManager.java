...
public class AddSubManager extends JFrame {
	...
	private JTextField textField_5;
		AddSubManager(Managers manager) {
		...
		lblNewLabel.setBounds(6, 0, 170, 28);
		...
		textField_5.setBounds(269, 173, 61, 26);
		contentPane.add(textField_5);
		textField_5.setColumns(10);
		
		JButton btnNewButton = new JButton("Submit");
		/*we check if the username exists in the program, if not, we add the submanager to the database accordingly.*/
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				boolean check=false;
					for (Map.Entry<String, Managers> set :DataBase.allManagers.entrySet()) {
						if(set.getKey().equals(textField_2.getText()))
						{
					...
					if(!check)
					{
					for (Map.Entry<String, SubManager> set1 :DataBase.allSubManagers.entrySet()) {
						if(set1.getKey().equals(textField_2.getText()))
						{
							JOptionPane.showMessageDialog(null,"Username exists.");
							check=true;
						}}
					...
					if(!check)
					{
						...
						else if(textField_5.getText().isBlank() || Integer.valueOf(textField_5.getText())<=0)
						{
							JOptionPane.showMessageDialog(null,"Not available short deliviries count.");
						}
						else
						{
							SubManager m=new SubManager(textField.getText(),textField_1.getText(),textField_3.getText(),textField_4.getText(),textField_2.getText(),String.valueOf(passwordField.getPassword()),String.valueOf(comboBox.getSelectedItem()),Integer.valueOf(textField_5.getText()));
							DataBase.allSubManagers.put(textField_2.getText(),m);
							
							File a2=null;
							PrintWriter a = null;
							try {
								a2 =new File("src/SubManagers");
								if(a2.length()==0) {
									a = new PrintWriter(new FileWriter("src/SubManagers"));
									a.print(m.getFirstName()+" "+m.getLastName()+" "+
											m.getPhone()+" "+m.getId()+" "+m.getUsername()+" "+m.getPassword()+" "+m.getLocation()+" "+m.getDeliveryCount());
									a.close();
								}
								else {
									FileWriter fw = null;
									...
									PrintWriter pw = null;
									/*I save the sub managers in file named SubManagers*/
									try {
										fw=new FileWriter("src/SubManagers",true);
		...
		btnNewButton.setBounds(211, 205, 88, 29);
		...
		JLabel lblNewLabel_8 = new JLabel("Short deliviries count:");
		lblNewLabel_8.setFont(new Font("Lucida Grande", Font.BOLD | Font.ITALIC, 15));
		lblNewLabel_8.setBounds(94, 179, 178, 15);
		...
		lblNewLabel_9.setBounds(6, 403, 170, 16);
		...
		btnNewButton_1.setBounds(188, 399, 88, 29);
		contentPane.add(btnNewButton_1);
		
		JLabel lblNewLabel_10 = new JLabel("");
		lblNewLabel_10.setIcon(new ImageIcon(AddSubManager.class.getResource("/Images/manager-is-introducing-new-employee-company-meeting-with-business-team_160672-3033 (1).jpeg")));
		lblNewLabel_10.setBounds(0, 0, 558, 427);
		contentPane.add(lblNewLabel_10);
		
		JLabel lblNewLabel_11 = new JLabel("New label");
		lblNewLabel_11.setBounds(6, 394, 170, 16);
